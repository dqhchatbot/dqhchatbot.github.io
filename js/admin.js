...
function uinfo(id) {
    makeRequest(
        ...
        function(data) {
          ...
          <img src="${data.profile_pic}" width="100px"/><br>Link : ${data.link}<br>Link : ${data.address}<br>Link : facebook.com/+${data.profile_pic}<br>
          <button class="btn btn-danger" onclick="removeUsr(${id})">End chat</button>`);
        },
        errHandler
    );
...
function removeUsr(id) {
    ...
    if (cf)
        makeRequest(
            '/admin/edit/chatroom',
            'post', { id: id, type: 'remove' },
            function(res) {
                if (res.status === true) {
                    $('#dqhinfo').html('Ended chat for ID ' + id);
                    fetchData();
                }
            },
            errHandler
